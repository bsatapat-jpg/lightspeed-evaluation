...
from lightspeed_evaluation.core.storage.protocol import StorageProtocol
...
def create_pipeline_storage_backend(
    ...
    entries become SQL backends; file entries use a no-op in the pipeline
    (file artifacts are produced by the output layer). Multiple backends
    are wrapped in a composite.

    Returns:
        A single ``StorageProtocol`` instance; never None.
    """
    ...
    for config in storage_configs:
        if isinstance(config, DatabaseBackendConfig):
            ...
            backends.append(NoOpStorageBackend(backend_name="file"))
